...
/**
 * Java中定义了变成参数，允许在调用方法时传入不定长度的参数。
 * 在定义方法时，在最后一个形象后加三个点... 就表示该形象可以接受多个参数值，多个参数值被当成数组传入
 * 上述定义有几个要点要注意：
 * 1.可变参数只能作为函数的最后一个参数，但其前面可以有也可以没有任何其他参数值
 * 2.由于可变参数必须是最后一个函数，所以一个函数最多只能有一个可变参数
 * 3.Java的可变残水，会被编译器转型为一个数组
